...
class Compiler (Parser) :
   ...
   def open_namespace (self, ns) :
       ns.item_name = ns.name
       ns.item_expand = True
       ns.item_icon = "namespace"
       self.copy_location (ns)
       self.enter (ns)
       self.selectColor (ns)
       self.markDefn (ns)

       self.openScope (ns)
       self.openCompletion (ns, outside = True)

   def close_namespace (self, ns) :
       self.closeCompletion (ns, outside = True)
       self.closeScope ()
   ...
   def end_class (self, cls) :
       self.closeRegion ()

   # enumeration type

   def open_enum (self, typ) :
       typ.item_name = typ.name
       typ.item_expand = True
       typ.item_icon = "enum"
       self.copy_location (typ)
       self.enter (typ) # add declaration into current scope
       self.selectColor (typ)
       self.markDefn (typ) # mark as definition (in editor)

       self.openScope (typ) # open scope for identifiers
       self.openCompletion (typ, outside = True) # area with text completion

   def close_enum (self, typ) :
       self.closeCompletion (typ, outside = True)
       self.closeScope ()
   ...
   def on_local_decl (self, stat) :
       self.on_simple_decl (stat.inner_decl)
